@php
    $section = optional(isset($sections['candidate-pathways']) ? json_decode($sections['candidate-pathways']) : null);
    $slug = 'candidate-pathways';
@endphp
<div class="card">
    <div class="card-body">
        <form method="POST" enctype="multipart/form-data" action="{{ route('admin.pages.savepage') }}">
            ...
            <div class="form-group row mb-3">
                ...
                <div class="col-lg-9">
                    <input type="text" name="{{ $slug }}[pathway_{{ $i }}_title]" class="form-control" value="{{ old($slug.'.pathway_'.$i.'_title', $section->{'pathway_'.$i.'_title'} ?? '') }}">
                </div>
            </div>
            <div class="form-group row mb-3">
                <label class="col-form-label col-lg-3">Description:</label>
                <div class="col-lg-9">
                    <textarea name="{{ $slug }}[pathway_{{ $i }}_desc]" class="form-control" rows="2">{{ old($slug.'.pathway_'.$i.'_desc', $section->{'pathway_'.$i.'_desc'} ?? '') }}</textarea>
                </div>
            </div>
            @endfor
